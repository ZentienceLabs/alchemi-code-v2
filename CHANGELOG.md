...
- Feature: Add DeepInfra as a model provider in Alchemi Code (#7661 by @Thachnh, PR by @Thachnh)
...
- Revert "feat: enable loading Alchemi modes from multiple files in .roo/modes directory" temporarily to fix a bug with mode installation
...
- Add "installed" filter to Alchemi Marketplace (thanks @semidark)
- Improve filesystem access checks (thanks @elianiva)
- Support for loading Alchemi modes from multiple YAML files in the `.roo/modes/` directory (thanks @farazoman)
...
- Fix issue where new users couldn't select the Alchemi Code Cloud provider (thanks @daniel-lxs!)

## [3.25.18] - 2025-08-19

- Add new stealth Sonic model through the Alchemi Code Cloud provider
...
- Remove "(prev Alchemi Cline)" from extension title in all languages (thanks @app/roomote!)
...
- Add Alchemi Code Cloud Waitlist CTAs (thanks @brunobergher!)
...
- Add Alchemi Marketplace to make it easy to discover and install great MCPs and modes!
...
- Update marketplace branding to 'Alchemi Marketplace' (thanks @SannidhyaSah!)
...
- Fix menu breaking when Alchemi is moved between primary and secondary sidebars (thanks @chrarnoldus!)
...
- Revert "Switch to the new Alchemi message parser" (appears to cause a tool parsing bug)
...
- Switch to the new Alchemi message parser for improved performance (thanks Cline!)
...
- You can now toggle the `terminal.integrated.inheritEnv` VSCode setting directly for the Alchemi Code settings (thanks @KJ7LNW)
...
- Fix a nasty bug that would cause Alchemi Code to hang, particularly in orchestrator mode
...
- Simplify and streamline Alchemi Code's quick actions
- Allow Alchemi Code settings to be imported from the welcome screen (thanks @julionav)
...
- Fix word wrapping in Alchemi message title (thanks @zhangtony239!)
...
- Focus Alchemi Code in the sidebar when running tasks in the sidebar via the API
- Improve subtasks UI
...
- Add command to focus Alchemi Code input field (thanks @axkirillov!)
...
- Allow processes to access the Alchemi Code API via a unix socket
...
- Add an activation command that other extensions can use to interface with Alchemi Code (thanks @gtaylor!)
...
- Pass current language to system prompt correctly so Alchemi thinks and speaks in the selected language

## [3.9.0] - 2025-03-18

- Internationalize Alchemi Code into Catalan, German, Spanish, French, Hindi, Italian, Japanese, Korean, Polish, Portuguese, Turkish, Vietnamese, Simplified Chinese, and Traditional Chinese (thanks @feifei325!)
- Bring back support for MCP over SSE (thanks @aheizi!)
- Add a text-to-speech option to have Alchemi talk to you as it works (thanks @heyseth!)
...
- Fix wheel scrolling when Alchemi is opened in editor tabs (thanks @GitlyHallows!)
...
- Add an option in the prompts tab to save tokens by disabling the ability to ask Alchemi to create/edit custom modes for you (thanks @hannesrudolph!)
...
- Add opt-in telemetry to help us improve Alchemi Code faster (thanks Cline!)
...
- Add support for a .rooignore to prevent Alchemi Code from read/writing certain files, with a setting to also exclude them from search/lists (thanks Cline!)
...
- Support running Alchemi in multiple editor windows simultaneously (thanks @samhvw8!)
...
- Add a new “Human Relay” provider that allows you to manually copy information to a Web AI when needed, and then paste the AI's response back into Alchemi Code (thanks @NyxJae)!
...
- Introducing Alchemi Code 3.7, with support for the new Claude Sonnet 3.7. Because who cares about skipping version numbers anymore? Thanks @lupuletic and @cte for the PRs!
...
- Fix system prompt to make sure Alchemi knows about all available modes
...
- Prompt tweaks to make Alchemi better at creating new custom modes for you

## [3.3.6]

- Add a "new task" tool that allows Alchemi to start new tasks with an initial message and mode
...
- New Alchemi Code community Discord at https://roocode.com/discord!
...
- **Name Change From Alchemi Cline to Alchemi Code:** We're excited to announce our new name! After growing beyond 50,000 installations, we've rebranded from Alchemi Cline to Alchemi Code to better reflect our identity as we chart our own course.

- **Custom Modes:** Create your own personas for Alchemi Code! While our built-in modes (Code, Architect, Ask) are still here, you can now shape entirely new ones:
...
- This release adds chat modes! Now you can ask Alchemi Code questions about system architecture or the codebase without immediately jumping into writing code. You can even assign different API configuration profiles to each mode if you prefer to use different models for thinking vs coding. Would love feedback in the new Alchemi Code Reddit! https://www.reddit.com/r/RooCode
...
- Alchemi Cline now allows configuration of which commands are allowed without approval!
...
- Alchemi Cline now runs in all VSCode-compatible editors
...
- Alchemi Cline now can run side-by-side with Cline

## [2.1.3]

- Alchemi Cline now allows browser actions without approval when `alwaysAllowBrowser` is true
