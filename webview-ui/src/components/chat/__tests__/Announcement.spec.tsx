	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			...
			if (key === "chat:announcement.stealthModel.note") {
				return "As a thank you for all the helpful feedback about Sonic, you'll also continue to have free access to the grok-code-fast-1 model for another week through the Alchemi Code Cloud provider."
			}
		...
		if (i18nKey === "chat:announcement.stealthModel.note") {
			return (
				<>
					...
					access to the grok-code-fast-1 model for another week through the Alchemi Code Cloud provider.
				</>
			)
		}
		return <>{children}</>
